...
2. 한국인에게 인기 있고 접근성 좋은 여행지 위주로 추천 (제주, 부산, 강릉 등 국내 여행지도 좋은 후보)
3. 지금 출발할 때의 계절·날씨 적합성을 반드시 고려 (우기·혹서기·혹한기인 곳은 피하고, 지금이 최적기인 곳을 우선)
4. 3곳은 서로 성격이 다른 여행지로 구성 (예: 미식 도시 / 휴양지 / 자연·문화 — 같은 유형 반복 금지)
5. 각 여행지의 특색있는 매력 포인트를 강조하고, bestFor는 사용자가 입력한 조건과 직접 연결
6. 짧은 기간(2~3일)에는 비행시간이 짧거나 국내인 곳 위주, 긴 기간에는 장거리도 후보

## 예산 기준 (1인 기준, 항공+숙박+활동 포함)
- 알뜰하게 (100만원 이하): 국내, 동남아, 일본 근교 등
...
- 출발 시기: ${new Date().getFullYear()}년 ${new Date().getMonth() + 1}월 (지금 기준)
