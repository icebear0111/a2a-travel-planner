...
const toMinutes = (time: string) => {
  const [hours = 0, minutes = 0] = (time || '0:0').split(':').map(Number);
  return hours * 60 + minutes;
};

// 귀환편 출발 시각에서 역산한 구체적 시각을 프롬프트에 명시하기 위한 헬퍼
// ("2.5시간 전" 같은 상대 표현은 모델이 자주 어긴다)
const minusMinutes = (time: string, minutes: number) => {
...
// 자정 넘김으로 간주해 24시간을 더한 키로 정렬한다.
const DAY_WRAP_THRESHOLD_MINUTES = 12 * 60;
...
function normalizeDayActivities(
  ...
  const keyed = activities.map((activity) => {
    const rawMinutes = toMinutes(activity.time);
    if (prevRawMinutes !== -Infinity && rawMinutes < prevRawMinutes - DAY_WRAP_THRESHOLD_MINUTES) {
      wrapOffset += 24 * 60;
    }
...
- Day 1 is the arrival day and Day ${intent.duration} is the departure day: assign them lighter areas near the city center or the arrival/departure hub.${
            intent.travelMode === 'car' || intent.useRentalCar
              ? `
- This is a DRIVE trip: split the region by direction/권역 (예: 제주 동부 / 서부 / 시내권) so each day is
  ONE drive loop in ONE direction from the base — days must never require crossing to the opposite side.`
              : ''
          }
...
const normalizePlaceKey = (value: string) =>
  value
    .toLowerCase()
    // "점심: X"와 "저녁: X"는 같은 장소다 — 라벨 접두어는 중복 판정에서 무시한다
    .replace(/^(아침|점심|저녁|중식|석식|카페|이동)\s*:\s*/, '')
    .replace(/[^\p{L}\p{N}]/gu, '')
...
// 여러 날에 같은 관광지·카페·쇼핑 스팟이 반복 편성되면 첫 등장만 남긴다
// (food는 제외 — 중복 제거 시 그날 식사가 통째로 비어 더 큰 품질 문제가 된다)
const CROSS_DAY_DEDUPE_TYPES = new Set(['sightseeing', 'theme', 'coffee', 'shopping']);
...
export function dedupeRepeatedPlaces(itinerary: DayItinerary[]): DayItinerary[] {
  ...
  return itinerary.map((day) => {
    const foodCount = day.activities.filter((activity) => activity.type === 'food').length;
    let removedFood = 0;
    ...
    return {
      ...
      activities: day.activities.filter((activity) => {
        ...
        if (!CROSS_DAY_DEDUPE_TYPES.has(activity.type) && !isFood) return true;

        const key = normalizePlaceKey(activity.title);
        if (!key) return true;
        if (isDuplicate(key)) {
          // 같은 식당의 반복 편성 — 단, 그날 다른 식사가 남을 때만 제거한다 (식사 공백 방지)
          if (isFood) {
            if (foodCount - removedFood <= 1) return true;
            removedFood += 1;
            return false;
          }
          return false;
...
[DRIVING TRIP]
- ${DRIVE_TRIP_PROMPT}
- Return towards the hotel ONLY at the END of the day — never drive back to the hotel's district
  for a mid-day meal or stop; eat inside today's area instead.`
    : '';
...
- Schedule light activities near the ${stationWord} area — **ALL sightseeing/meals must END by ${minusMinutes(flight.returnTime, 40)}.**${rentalReturnInstruction}
- **Schedule "${stationWord}으로 이동" (Type: 'transport') starting at ${minusMinutes(flight.returnTime, 40)}** (40 minutes before the ${flight.returnTime} departure).
...
- **Activity #1 MUST be hotel checkout & luggage storage (Type: 'hotel', around 09:00, 30m).**
- Schedule light activities near the airport or central area — **ALL sightseeing/meals must END by ${minusMinutes(flight.returnTime, 90)}.**${rentalReturnInstruction}
- **Schedule "공항으로 이동" (Type: 'transport') starting at ${minusMinutes(flight.returnTime, 90)}** (1.5 hours before the ${flight.returnTime} domestic flight).
